...
/* Presentational job-detail markup, shared by the SSR page (public jobs) and the client
   PrivateJobView fallback (private/invite-only jobs, which need the viewer's auth token). */
export default function JobDetailBody({ job }: { job: Job }) {
        ...
        <div className="mx-auto max-w-screen-2xl px-6 pb-12 pt-8">
          ...
          <div className="mt-3 flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="flex shrink-0 items-center gap-2">
              {job.is_private && (
                <span className="glass inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white/90">دعوة خاصة</span>
              )}
              <ReportButton kind="job" id={job.id} label="إبلاغ"
                className="glass inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-white/90 transition hover:text-white" />
            </div>
          </div>
